...
bool TaoHocKy(ListNamHoc& l)
{
	...
	do {
		...
		if (flagbd)
		{
			...
			if (!sosanhNgay(Dnow, ngBD))//so sanh voi tg hien tai
			{
				setColor(background_color, red);
				printA_Sentence("! NGAY BAT DAU PHAI SAU NGAY HIEN TAI !", HEIGHT - 8);
				_getch();
				continue;
			}
			setColor(background_color, title_color1);
		...
		{
			setColor(background_color, title_color1);
			printA_Sentence("- NGAY BAT DAU :   " + to_string(ngBD.d) + "/" + to_string(ngBD.m) + "/" + to_string(ngBD.y), 11);
		}

		setColor(background_color, title_color1);
		printA_Sentence("- NGAY KET THUC -", 13);
		nhapNgay_ve(15);
		ngKT = nhapNgay_nhap(15);
	...
	if (nodeNam)
	{
		...
		if (f.is_open() && (f >> s) && (s != " "))
		{
			f.close();
			setColor(background_color, red);
			printA_Sentence("! KI HOC DA DUOC TAO !", HEIGHT - 5);
			_getch();
			return false;
		}
		HocKy* hockyP = NULL;
		...
		if (flag)
		{
			...
			setColor(background_color, text_color);
			printA_Sentence("Xung dot thoi gian voi hoc ki truoc", HEIGHT / 2);
			_getch();
			return false;
		}
		hockyP->headMon = NULL;
		...
		f.close();
		setColor(background_color, title_color);
		printA_Sentence("~ TAO MOI HOC KI THANH CONG ~", HEIGHT / 2 - 1);
		_getch();
		return true;
	}
...
void TaoMon(ListNamHoc& l, int nam)
{
	...
	printA_Sentence("~ TAO MOI MON HOC ~", 5);
	setColor(background_color, title_color1);
	printA_Sentence("Luu y: Cac mon hoc trong danh sach truoc, neu da duoc tao thi se bi bo qua", HEIGHT - 4);
	if (!Ask_YN("Ban that su muon tao moi nhung mon hoc nay?"))
	{
		system("cls");
		setColor(background_color, red);
		printA_Sentence("! TAO MOI MON HOC THAT BAI !", HEIGHT / 2 - 1);
...
void CapNhatDiemSv(ListNamHoc& l)
{
	...
	while (true)
	{
		...
		string filename = to_string(nam) + "hk" + to_string(ki) + pMon->data.tenMon + ".txt";
	...
	//		break;
	//}
	//khong can vi da hien thi danh sach thi chac chan co
	//if (node_sv_mon == NULL)
	//{
		...
		if (line.find(mon->data.id + ",") == 0)
		{
			...
			else
			{
				file << "\n" << mon->data.id << ',' << mon->data.tenMon << ',' << mon->data.tenGv << ',' << mon->data.so_tc << ','
					...
					<< mon->data.MaxSv << ',';
				setColor(background_color, title_color1);
				printA_Sentence("~ CAP NHAT MON HOC THANH CONG ~", HEIGHT - 10);
				success = true;
			}
		}
		else
...
void CapNhatMonHoc(ListNamHoc& l)
{
	system("cls");
	setColor(background_color, title_color);
	printA_Sentence("~ CAP NHAT MON HOC ~", 5);
	int nam, stt_hk;
	NodeMon* node_mon = NULL;
	node_mon = timNodeMon_CapNhat(l, nam, stt_hk, node_mon);//cap nhat ko can dung node HeadmonHk, chi gan vao cho du tham so
	if (node_mon == NULL)
	{
		_getch();
		return;
	...
	do
	{
		...
		else
		{
			...
			GhiFileMon(to_string(nam) + "hk" + to_string(stt_hk) + ".txt", node_mon, remove);
			break;
		}
	} while (true);
}
void XoaMonHoc(ListNamHoc& l)
{
	...
	NodeMon* node_mon = timNodeMon_CapNhat(l, nam, stt_hk, headmonHK);
	if (node_mon == NULL)
	{
		_getch();
		return;
	...
	if (node_mon->headSvMon != NULL && !XoaMon_sv(l, headmonHK, node_mon, nam, stt_hk))
	{
		setColor(background_color, red);
		printA_Sentence("XOA MON " + node_mon->data.id + " THAT BAI!!!", HEIGHT - 10);
		setColor(background_color, title_color1);
		printA_Sentence("<-- Nhan phim bat ki de quay lai", HEIGHT - 5);
		_getch();
		return;
	...
	*p = next;
	...
	setColor(background_color, title_color1);
	printA_Sentence("~ XOA MON HOC THANH CONG ~", HEIGHT - 10);
	setColor(background_color, title_color1);
	printA_Sentence("<-- Nhan phim bat ki de quay lai", HEIGHT - 5);
	_getch();
}
NodeMon* timNodeMon_CapNhat(ListNamHoc& l, int& nam, int& ki, NodeMon*& headmonHK)
{
	...
	nam = nodeNam->data.tg.ngay_bd.y;
	system("cls");
	ki = chonKi("Chon hoc ki:");
	int c = viewDsMonHk(nodeNam->data.hk[ki - 1].headMon, lc);
	if (c == 0)
	{
		end(lc);
		return NULL;
	...
	if (lc == 0)
		...
		return timNodeMon_CapNhat(l, nam, ki, headmonHK);  // headmonHK la node tra ve
...
bool XoaMon_sv(ListNamHoc& l, NodeMon*& headmonHK, NodeMon* mon, int nam, int ki)
{
	...
	if (mon->headSvMon)
	{
		...
		if (Ask_YN("Mon nay da duoc mo cho sinh vien dang ky! Ban that su muon xoa?"))
		{
			...
			{
				//tim node lop cua sv
				int year = nam % 2000 - stoi(temp->lop.substr(0, 2)) + 1;//sv nam 1 2 3 4
